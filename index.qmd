...
### NONpractitioners, items as categorical, Polychoric, mindfulness nonpractitioners only

```{r one_factor_ordered_polychoric_nonpractitioners}
...
#### Latent correlation of the two subfactors
...
```{r semplot-model_two_dim_wo_13_ordered_poly}
...
```{r semplot-model_two_dim_wo_13_ordered_poly-meditators-only}
semPaths(model_two_dim_wo_13_ordered_poly,
         what = "path",
         whatLabels = "stand",
         layout = "circle",
...
title("Two factors, ordered, polychoric - mindfulness practitioners only")
```
...
## NONpractitioners, Two correlated factors, individuals withOUT meditation practice only

### items as numeric

```{r model_two_dim_wo_13_nonmeditators_only}
...
```{r model_two_dim_wo_13_nonmeditators_only_categorical}
...
```{r model_two_dim_wo_13_nonmeditators_only_categorical_polychoric}
...
## Testing essential tau equivalence
...
### Comparing model fit between congeneric and tau equivalence - whole FMI13R
...
```{r anova-model_two_dim_wo_13_ordered_poly--model_two_dim_wo_13_ordered_poly_tau_equiv}
...
```{r effect-size-congeneric-minus-tau-equivalence}
...
### POLY - Comparing model fit between congeneric and tau equivalence - each subscale individually and for polychoric items


#### Definition - Congeneric Model

```{r FMI13R-each-subscale-congeneric}
...
#### CFA for Presence and Acceptance - Congeneric


```{r model_presence_wo_13_ordered_poly_congeneric}
...
```{r model_acceptance_wo_13_ordered_poly_congeneric}
...
#### Definition - Tau equivalent model



```{r FMI13R-each-subscale-tau-equivalent}
...
#### CFA for Presence and Acceptance - Tau equivalent (polychoric)

##### Presence

```{r model_presence_wo_13_ordered_poly_tau_equiv}
...
```{r model_acceptance_wo_13_ordered_poly_tau_equiv}
...
```{r anova-model_presence_wo_13_ordered_poly_--model_presence_wo_13_ordered_poly_tau_equiv}
...
```{r effect-size-congeneric-minus-tau-equivalence-presence}
...
```{r anova-model_acceptance_wo_13_ordered_poly_--model_acceptance_wo_13_ordered_poly_tau_equiv}
...
#### Effect size - Acceptance

```{r effect-size-congeneric-minus-tau-equivalence-acceptance}
...
### NUM - Comparing model fit between congeneric and tau equivalence - each subscale individually and for NUMERIC items



#### CFA for Presence and Acceptance - Congeneric, numeric items


```{r model_presence_acceptance_wo_13_numeric_congeneric}
# presence:
...
cfa_results[["FMI-13R, Presence, numeric, congeneric"]] <-
  get_results_list(model_presence_wo_13_numeric_congeneric,
                   "FMI-13R, Presence, numeric")
...
cfa_results[["FMI-13R, Acceptance, numeric, congeneric"]] <-
  get_results_list(model_acceptance_wo_13_numeric_congeneric,
                   "FMI-13R, Acceptance, numeric")
```
...
#### CFA for Presence and Acceptance - Tau equivalent, numeric items

##### Presence

```{r model_presence_wo_13_numeric_tau_equiv}
...
```{r model_acceptance_wo_13_numeric_tau_equiv}
...
#### Significance - Presence, numeric

Do not subtract the Likelihoods of the two models. Instead, the *Satorra-Bentler scaled chi-square difference test* should be used to compare the models:


```{r anova-model_presence_wo_13_numeric_--model_presence_wo_13_numeric_tau_equiv}
...
#### Effect size - Presence, numeric

```{r effect-size-congeneric-minus-tau-equivalence-presence-numeric}
...
```{r anova-model_acceptance_wo_13_numeric_--model_acceptance_wo_13_numeric_tau_equiv}
...
```{r effect-size-congeneric-minus-tau-equivalence-acceptance-numeric}
...
All models show congeneric measurement assumptions, if not stated otherwise.
...
cfa_results_df |> 
  ...
  mutate(Model = str_replace_all(Model, "mindfulness ", "")) |> 
    mutate(Model = str_replace_all(Model, ", congeneric ", "")) |> 
...
```{r cfa-results-table-interactive-reactable}
#| eval: true
...
```{r cfa-results-table-interactive-datatable}
#| results: asis
...
### Joint Table transposed

```{r invariance_analysis_results_anova_df_transposed}
